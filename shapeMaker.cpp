...
Wm5::TriMesh* ShapeMaker::CreateTitle()
{
    VertexFormat* vformat;
    if (mUseBumpMap)
    {
    ...
    std::string baseName = Environment::GetPathR("title.wmtf");
    Texture2D* baseTexture = Texture2D::LoadWMTF(baseName);
    baseTexture->GenerateMipmaps();
/*
    if (mUseBumpMap)
...
Wm5::TriMesh* ShapeMaker::CreateJoke()
{
    VertexFormat* vformat;
    if (mUseBumpMap)
    {
    ...
    std::string baseName = Environment::GetPathR("torus.wmtf");
    Texture2D* baseTexture = Texture2D::LoadWMTF(baseName);
    baseTexture->GenerateMipmaps();
/*
    if (mUseBumpMap)
